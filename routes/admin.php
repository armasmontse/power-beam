...
/*empresas*/
Route::group(['middleware' => ['permission:manage_companies'] ,'prefix' => 'companies', 'as' => 'companies.' ], function(){

    Route::get( '/' , 'Admin\Users\ManageCompanyController@indexView')->name('index');

    Route::group(['middleware' => ['onlyajax'], 'as' => 'ajax.'  ,'prefix' => 'ajax' ], function(){
        Route::resource('/','Admin\Users\ManageCompanyController',[
...
Route::group(['middleware' => ['permission:manage_projects'] ,'prefix' => 'projects', 'as' => 'projects.' ], function(){
	...
	Route::group(['prefix' => '{projects_admin}'], function(){
		...
		Route::patch('/take', 'Admin\Projects\ManageProjectsController@take')->name('take');
		Route::post('/quoteFile', 'Admin\Projects\ManageProjectsController@quoteFile')->name('quoteFile');
		Route::post('/addNote', 'Admin\Projects\ManageProjectsController@addNote')->name('addNote');
		Route::patch('/next', 'Admin\Projects\ManageProjectsController@next')->name('next');
		Route::post('/outputFile', 'Admin\Projects\ManageProjectsController@outputFile')->name('outputFile');
		Route::post('/error', 'Admin\Projects\ManageProjectsController@error')->name('error');
		Route::post('/placementFile', 'Admin\Projects\ManageProjectsController@placementFile')->name('placementFile');
		Route::get('/download-files', 'Admin\Projects\ManageProjectsController@downloadFiles')->name('downloadFiles');

		Route::group(['prefix' => 'quotes', 'as' => 'quotes.'], function(){

			Route::get('{admin_quote}', 'Admin\Quotes\ManageQuotesController@show')->name('show');
